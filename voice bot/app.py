...
def chat():
    ...
    try:
        ...
        if grievance_session.get("active") and grievance_session.get("collecting"):
            result = handle_grievance_turn(session_id, user_input, user_language, request_info=request_info)
        ...
        if booking_session.get("active"):
            result = handle_booking_turn(session_id, user_input, user_language, request_info=request_info)
            audio_output = text_to_speech(result["message"], result["lang"])
            return jsonify({
                "response": result["message"],
                "lang": result["lang"],
        ...
        if g_offer.get("offer_pending"):
            ...
            if intent["intent"] == "grievance_confirm":
                ...
                result = handle_grievance_turn(session_id, "", user_language, request_info=request_info)
                audio_output = text_to_speech(result["message"], result["lang"])
                return jsonify({
                    "response": result["message"],
                    "lang": user_language,
        ...
        if b_offer.get("offer_pending"):
            ...
            if intent["intent"] == "booking_confirm":
                ...
                result = handle_booking_turn(session_id, "I want to start booking a community hall.", user_language, request_info=request_info)
                audio_output = text_to_speech(result["message"], result["lang"])
                return jsonify({
                    "response": result["message"],
                    "lang": user_language,
...
def handle_booking_turn(session_id, user_input, lang, request_info=None):
    ...
    user_info = None
    if request_info:
        token = request_info.get("authToken")
        user_info = request_info.get("userInfo")

    # If no portal token, fall back to phone number as token
    active_token = token or mobile

    # Determine tenant_id dynamically from user_info, falling back to pb.amritsar or pg.mohali
    tenant_id = "pg.mohali"  # default matching the portal/grievance config
    if user_info:
        u_tenant = user_info.get("tenantId")
        if u_tenant:
            if "." in u_tenant:
                tenant_id = u_tenant
            elif u_tenant == "pb":
    ...
    prefill = {}
    if user_info:
        prefill["applicantName"] = user_info.get("name") or user_info.get("userName")
        prefill["applicantMobileNo"] = user_info.get("mobileNumber") or mobile
        prefill["applicantEmailId"] = user_info.get("emailId")
        prefill["userUuid"] = user_info.get("uuid")
    elif mobile:
        prefill["applicantMobileNo"] = mobile

    if prefill:
        payload["prefill_fields"] = prefill
...
def handle_grievance_turn(session_id, user_input, lang, request_info=None):
    ...
    if step == "START":
        ...
        # Check if real authToken and userInfo are provided in request_info
        req_auth_token = None
        req_user_info = None
        if request_info:
            req_auth_token = request_info.get("authToken")
            req_user_info = request_info.get("userInfo")
        ...
        if req_auth_token and req_user_info:
            # Bypass phone collection and OTP verification with real credentials from portal
            session["data"]["mobile"] = req_user_info.get("mobileNumber") or mobile or "9999999999"
            session["data"]["auth_token"] = req_auth_token
            session["data"]["user_info"] = req_user_info
            ...
            # Bypass phone collection and OTP verification with local phone fallback
